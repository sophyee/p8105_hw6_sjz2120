...
homicides_df = homicides %>%
  mutate(city_state = paste(city, state, sep = ", "),  # Create `city_state` and `solved` variables
         solved = ifelse(disposition %in% c("Closed by arrest", "Closed without arrest"), TRUE, FALSE)) %>%
  filter(!(city_state %in% c("Dallas, TX", "Phoenix, AZ", "Kansas City, MO", "Tulsa, AL")))

homicides_df
```

The cleaned `homicides_df` dataset contains `r nrow(homicides_df)` rows/observations and `r ncol(homicides_df)` columns/variables. Each of the `r nrow(homicides_df)` rows represents a homicide. The `r ncol(homicides_df)` key variables in this dataset include: `r colnames(homicides_df)`:
...
* `solved` -- indicates whether the homicide is solved (`TRUE` if solved, `FALSE` if unsolved)
